...
, mComputerPieces(12)
{
    ...
    for (int i = 5; i < size; ++i)
        for (int j = 0; j < size; ++j)
            if ((i + j) % 2 == 1)
                mBoard[i][j] = "w" + std::to_string(i) + std::to_string(j);
}
    ...
    while (true)
    {
        ...
        if (mPlayerTurn)
        {
            ...
            if (moves.size() == 0)
            {
                if (mComputerPieces > mPlayerPieces)
                    std::cout << "No available moves, YOU LOSE" << std::endl;
                else
                    std::cout << "No available moves, DRAW" << std::endl;
                break;
            }
            Move newMove;
            std::cout << "Select piece (i j): " << std::endl;
        ...
        else
        {
            ...
            if (firstPCMove.size() == 0)
                if (mComputerPieces < mPlayerPieces)
                    std::cout << "No available moves, YOU WIN" << std::endl;
                else
                    std::cout << "No available moves, DRAW" << std::endl;
            ...
            {
                std::cout << "Computer couldn't make move" << std::endl << "YOU WIN" << std::endl;
                break;
            }

            makeMove(bestMove, false);
            std::cout << "Computer has moved from (" << bestMove.oldI << ", " << bestMove.oldJ << ") to (" 
